...
%% Figure
figure
sgtitle('Energy Deviation [Amplitude = 50, numerical]', 'FontSize', 24, 'Interpreter', 'latex');
for i = 1:m
    ...
    %% Plot
    E_Ges_Rel = load([Name_of_Value, Name_ID{i,:}, '.txt']);
    subplot(3,2,i);
    hold on
    plot((0:deltaT:(length(E_Ges_Rel)*deltaT)-deltaT)*t_star,E_Ges_Rel(1:length(E_Ges_Rel)), shape(i), 'color', colors(i,:), 'MarkerSize', 15, 'LineWidth', 2.5);
    grid on;
    grid minor;
    xlim([0, length(E_Ges_Rel)*deltaT*t_star]);
    ylim([-1.5, 0.5]);
    xlabl = xlabel('$t$', 'FontSize', 20, 'Interpreter', 'latex');
    ylabl = ylabel('${E}_{Dev}\%$', 'FontSize', 20, 'Interpreter', 'latex');
    set(gca, 'FontSize', 20);
    power = i-3;
