...
% Naming for plots
s1 = 'Energy_Convergence_Temporal_BDF2_';
s3 = date;
%% Files Loading Path
% Figure on 
figure; hold on
%% Iteriere über alle Gruppen und speichere Zeros mit passendem Namen
    ...
    Init_2_Zeros   = load([base_path_1, '\Int_', CutOffDegree,'_',Initial_Deformation ,'_',TimeSteppScheme,'_',TimeStep{dT},'_Zeros.txt']);
    nameTag = [CutOffDegree '_' Initial_Deformation '_'  Amplitude{amplitude_column}];
    full_path = fullfile(base_path_2,'\', TimeSteppScheme, '\', TimeStep{dT},'\',nameTag ); % Kombiniere den Pfad mit der Name_ID
    Energy_Rel_Num_ = load([Name_of_Value, nameTag,'_',TimeSteppScheme, '_', TimeStep{dT}, '.txt']);
    %% Entdimensionalisierung
   Volumen_abs     = load([Name_of_Vol_Abs, nameTag,'_',TimeSteppScheme, '_', TimeStep{dT}, '.txt']);
    %R0              = ((3*Volumen_abs(1))/(4*pi))^(1/3);
    R0              =  1/(sqrt(pi)*2); % Non-dimensionalisation
    t_star          =  sqrt((0.07275)/(R0^3))'; 
    %% Loading Time Steps
    Zeitschritte_   = load(['..\Vector_Storages\', TimeSteppScheme, '\', TimeStep{dT} ,'\', nameTag, '\', nameTag,   '_GesamtZeitschritt_Speicher1.txt']);
    t               = [0 cumsum(Zeitschritte_(1:end-1))];
    %% LinePLots
    plot(t(1:Init_2_Zeros(amplitude_column,5)).*t_star,Energy_Rel_Num_(1:Init_2_Zeros(amplitude_column,5)),Format{dT},'color',Colors{dT},'linewidth',2)
    if dT == 1
    labelStr = '$\Delta t = 0.000277$';
    elseif dT == 2
    labelStr = '$\Delta t = 0.00055$';
    elseif dT == 3
    labelStr = '$\Delta t = 0.0011$';
    elseif dT == 4
    labelStr = '$\Delta t = 0.0022$';
    end
    legendEntries1{end+1} = [labelStr];
    end
...
xlim([0 4.75])   % Beschränkt die x-Achse von 0 bis 5
set(gca,'Fontsize',14); 
grid on
grid minor
