...
%% Testing Field
    for Zeitschritt = 1:1:Init_2_Zeros_0_004(amplitude,5)
...
if all([e1 e2 e3] > 0) && all(isfinite([e1 e2 e3]))
    ...
    R2(amplitude) = 1 - SSres/SStot;
        ...
        h(amplitude) = plot(NaN, NaN, Makers{amplitude}, 'Color', Colors{amplitude}, 'MarkerSize', 15, 'LineWidth', 2, 'DisplayName', ['Amplitude = ', Amplitude{amplitude}]);
        else
...
legendEntriesRef = [legendEntries, {'fitted line'}];
...
Titl = title('b) Temporal convergence','FontSize',14,'Interpreter','latex');
lgd = legend([h; h_ref], legendEntriesRef, 'Location','southeast','FontSize',14,'Interpreter','latex');
xlim([0.001 0.004]./t_non_dimension)
ylim([0.000006 0.0007])
grid on
grid minor