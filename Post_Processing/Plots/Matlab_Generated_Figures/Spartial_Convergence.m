...
%% Error
    % Figure Data
        ...
        err_values = err_values(sortIdx)*100/Zeros(amplitude,5);       %% WICHTIG! Devide by the number of Timesteps and mutiply with 100%
%% Slopes
...
%% Legend
        ...
        % Proxy für Legende (entspricht Optik von amplitude==1 && 6)
            ...
            h(amplitude) = plot(NaN, NaN, Makers{amplitude}, ...
                ...
                'DisplayName', ['Amplitude = ', num2str(str2double(Amplitude{amplitude})/100)]);
        end
...
% Legende: automatisch O(e^{-k}), mit zwei Nachkommastellen
% legendEntriesRef = [legendEntries, {sprintf('$O(e^{-%0.2f L})$', k_mean)}];
...
Titl = title(TIMESTEP,'FontSize',14,'Interpreter','latex');
...
cd '..\..\Plots\Code'
s0 = '..\Figures\';
s = strcat(s0,s1,'Init',s2,TimeSteppScheme,'_',TimeStep,'_',s3);
print2eps(s,gcf())