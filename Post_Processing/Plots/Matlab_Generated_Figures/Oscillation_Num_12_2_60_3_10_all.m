...
%% Akbari && Noori
%% Oscillation plots
%% For Coupled Initial Deformation:  
%% Initialisierung
clc 
clear all
...
colors = [0 0 0]/255;
% Naming for plots
s1 = 'Oscillation_Plots_Init_2_and_3_';
s2 = 'Amplitude_2_60_3_10_all_';
s3 = date;
%% Files Loading Path
...
EndPosition = col(35);
% Runden, damit es durch 50 teilbar wird
EndPositionRounded = round(EndPosition / 50) * 50;
% Berechnung des Schritts
stepSize = EndPositionRounded / 50;
...
subplot(5, 12, [1, 2, 13, 14]); % Belegt 2 Zeilen und 2 Spalten
...
for timestemp =1:stepSize:EndPositionRounded
    if plotPostion == 12 ||  plotPostion == 24 || plotPostion == 36 || plotPostion == 48 
    ...
    x = R_rho.*zetaa;               % convert to Cartesian coordinates
    PlotNum = 1 + PlotNum;
    subplot(5, 12, plotPostion); % Belegt 2 Zeilen und 2 Spalten
    plot(y,x,'k',-y,x,'k','LineWidth',3);    % Line plot
