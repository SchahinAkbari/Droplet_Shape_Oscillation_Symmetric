...
for groupIdx = 1:length(Names_Groups)
    ...
    for fileIdx = 1:numFiles
        ...
        Energy_Rel_Num_ = load([Name_of_Value, currentGroup{fileIdx},'_', TimeSteppScheme, '_', TimeStep, '.txt']);
        ...
        [MaxDeviation, Position_Max_Deviation] = max(abs(Energy_Rel_Num_(1:zeros(fileIdx,5))));
        if fileIdx == 1
        ...
        ylabl = ylabel('$\mathcal{E}_{E}$ in $\%$','FontSize',14, 'Interpreter','latex');
        ...
        subplot(1,2,2);         hold on
          ...
          ylabl = ylabel('$\mathcal{E}_{E,\max}$ in $\%$','FontSize',14, 'Interpreter','latex');
    end
...
lgd = legend(legendEntries2, 'Location', 'NorthEast', 'NumColumns', 1, 'Interpreter', 'latex');
