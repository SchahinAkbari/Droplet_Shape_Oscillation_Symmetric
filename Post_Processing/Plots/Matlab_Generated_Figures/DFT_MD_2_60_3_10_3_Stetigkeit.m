...
A_star = A/A00;
CutOff = length(A_star)-25000;     % Cut Off, after 5 Oscillation 
...
for i = 3:4
    ...
    set(hb,'Visible','off')
    set(gca,'yscal','log')
    xticks([H(1) H(2) 2*H(1) 2*H(2)])
    xticklabels({'f_2^*', 'f_3^*', '2f_2^*','2f_3^*'})
    xlim([0 0.45*H(11)])
    yticks([0.0001 0.001 0.01 0.1 1])
    yticklabels({'10^{-4}', '10^{-3}', '10^{-2}', '10^{-1}', '10^{0}'})
    grid on
    grid minor
    legendEntry2 = ['$a^*_', num2str(i-1), '(t) (a_2(0)=0.60 , a_3(0)=0.10)$'];
...
s = strcat(s0,s1,s2,'_',subbplot,'_',TimeSteppScheme,'_',TimeStep,'_',s3);
print2eps(s,gcf())
end
